export default {
  methods: {
    genInputBox () {
      ...
      if (this.multiple && this.cacheValue.length > 0) {
        // show clear button for multiple selection when there is selected value
        ...
        // otherwise shows caret (except without options)
        icon = this.$createElement('vi-icon', {
    ...
    genItem (option) {
      ...
      let children
      if (this.multiple) {
        const checkbox = this.$createElement('vi-checkbox', {
          props: {
            inputValue: this.isSelected(option)
          }
        })
